...
export function AuthProviders({ next = '/dashboard' }: Props) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full flex items-center justify-center gap-3 rounded-full border border-border/60 bg-white dark:bg-neutral-900 px-5 py-3 text-sm font-medium text-foreground shadow-sm hover:bg-muted transition-colors disabled:opacity-60"
    >
      {loading ? (
